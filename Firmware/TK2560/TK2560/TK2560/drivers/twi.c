...
// if twi_timeout_us == 0 then timeout checking is disabled
...
/* 
 * Function: twi_init
 * Desc    : initialize TWI pins and sets TWI bitrate
 * Input   : none
 * Output  : none
 */
void twi_init(void)
...
// 	bitClear(TWSR, TWPS0);
// 	bitClear(TWSR, TWPS1); // Prescaler value =1
// 	TWBR = ((F_CPU / TWI_FREQ) - 16) / 2;
	//TWBR =72;
	
	TWBR = 2;
	TWSR |=((1<<TWPS1)|(1<<TWPS0));//64 prescaller
	//TWSR =0b00000011;
	
	/* TWI bit rate formula from datasheet pg 242
...
void twi_disable(void)
{
  ...
  // deactivate internal pullups for TWI pins.
  digitalWrite(SDA, 0);
  digitalWrite(SCL, 0);
}
 ...
 * Desc     : attempts to become TWI bus master and read a series of bytes from a device on the bus
...
uint8_t twi_readFrom(uint8_t address, uint8_t* data, uint8_t length, uint8_t sendStop)
{
	...
	if (true == twi_inRepStart) {
		...
		// since the ISR is ASYNC, and we could get confused if we hit the ISR before cleaning up. 
		// Also, don't enable the START interrupt. There may be one pending from the
 ...
 * Desc    : attempts to become TWI bus master and write a series of bytes to a device on the bus
 * Input   : address: 7bit i2c device address
 *              data: pointer to byte array
 ...
 * Desc    : this gets called whenever a while loop has lasted longer than
