...
uint8_t DS1307Read(uint8_t address,uint8_t *data)
{
	...
	//Send the address of required register	
	res = twi_writeTo(104, &address, 1, true,false);
	//Error
	if(res > 0)	return 0;
		
	res = twi_readFrom(104, data, 1, true);
	//no data read
	if(!res)	return 0;
	//STOP
...
uint8_t DS1307Write(uint8_t address,uint8_t data)
{
	...
	//Now write the value
	res = twi_writeTo(104, &data, 1, true,true);
	//Error
	if(res>0)	return 0;
...
uint8_t ClockInit()
{
	...
	LCD_PrintString("Clock init1",2,1);
	//Clear CH Bit
	temp&=(~(1<<CH));
